<template>
	<div ref="layoutElement" class="layout-cards" :style="{
		'--size': width >= 600 ? size : 1,
	}">
		<template v-if="loading || itemCount > 0">
			<cards-header v-model:size="sizeWritable" v-model:selection="selectionWritable" v-model:sort="sortWritable"
			...
			<div class="grid" :class="{ 'single-row': isSingleRow }">
				<card v-for="item in items" :key="item[primaryKeyField.field]" v-model="selectionWritable"
					:item-key="primaryKeyField.field" :collection="collection" :item="item" :imageFit="imageFit"
					:icon="icon" :tag="tag" :idShow="idShow" :title="title" :subtitle="subtitle"
